...
[**myGroups**](GroupAPI.md#mygroups) | **GET** /group/myGroups | 
...
# **myGroups**
```swift
    open class func myGroups(pulsarrApiKey: String, completion: @escaping (_ data: [GroupDTO]?, _ error: Error?) -> Void)
```
...
GroupAPI.myGroups(pulsarrApiKey: pulsarrApiKey) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
